...
class TemplatedEmail(EmailMultiAlternatives):
    ...
    def should_self_render(self):
        return self.is_templated() and (
            settings.EMAIL_FORCE_SELF_RENDER or settings.EMAIL_BACKEND != "anymail.backends.postmark.EmailBackend"
        )
    ...
    def _render(self):
        if not self.should_self_render():
            ...
            with open(
                os.path.join(settings.ROOT_DIR, "seedorf", "templates", "emails", html_template_filename)
            ) as file:
                html_template = file.read()
            with open(
                os.path.join(settings.ROOT_DIR, "seedorf", "templates", "emails", text_template_filename)
            ) as file:
