...
if [[ $# -eq 0 ]] ; then
    echo 'usage:'
    echo '    docker-compose -f docker-compose-production.yml run postgres restore <backup-file>'
    echo ''
    echo 'to get a list of available backups, run:'
    echo '    docker-compose -f docker-compose-production.yml run postgres list-backups'
    exit 1
fi
